...
for row in rows:
    ...
    for attempt in range(3):
        try:
            r=requests.post(f'{base}/v1/chat/completions',headers={'Authorization':f'Bearer {key}','Content-Type':'application/json'},json={'model':'gpt-5-nano','messages':[{'role':'system','content':'你是謹慎的國中國文教師，只輸出 JSON。'},{'role':'user','content':prompt}], 'max_completion_tokens': 1400},timeout=120)
            payload=r.json()
