    ...
    <main className="min-h-screen overflow-hidden bg-paper text-ink">
      ...
      <section id="classics" className="mx-auto grid max-w-[1440px] gap-8 px-5 py-16 lg:grid-cols-[220px_1fr] lg:px-16 lg:py-24">
        ...
        <div className="paper-page relative min-w-0 border-y border-ink/10 p-6 sm:p-10 lg:border-y-2 lg:p-14">
          ...
          <div className="max-w-4xl"><div className="flex flex-wrap items-end justify-between gap-5"><div><p className="section-kicker">{active.theme}</p><h2 className="mt-3 font-serif text-4xl font-bold tracking-tight sm:text-5xl">{active.title}</h2><p className="mt-3 font-sans text-sm text-ink/55">{active.author}・{active.dynasty}</p></div><div className="flex flex-wrap gap-2"><button className="listen-button" onClick={() => speak(active.sections.map((item) => item.original).join(" "))}><Headphones size={16} />朗讀全文</button><button className={`bookmark-button ${favorite(`classic:${active.id}`) ? "is-active" : ""}`} onClick={() => toggleFavorite(`classic:${active.id}`)} aria-pressed={favorite(`classic:${active.id}`)}>{favorite(`classic:${active.id}`) ? <BookmarkCheck size={16} /> : <Bookmark size={16} />}{favorite(`classic:${active.id}`) ? "已收藏" : "收藏篇章"}</button></div></div>
