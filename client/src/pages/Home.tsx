...
import { BookOpen, Bookmark, BookmarkCheck, ChevronRight, Clock3, Headphones, Info, Library, MapPin, Menu, Play, RotateCcw, Search, SlidersHorizontal, Square, Trash2, Volume2, X } from "lucide-react";
...
export default function Home() {
  ...
  const [quizHistory, setQuizHistory] = useState<QuizAttempt[]>(() => { try { return JSON.parse(localStorage.getItem("guowen-quiz-history") ?? "[]"); } catch { return []; } });
  const [wrongBook, setWrongBook] = useState<WrongQuestion[]>(() => { try { return JSON.parse(localStorage.getItem("guowen-wrong-book") ?? "[]"); } catch { return []; } });
  ...
  useEffect(() => { setTangSection(0); setQuizOpen(false); setQuizAnswers({}); setQuizSubmitted(false); window.speechSynthesis?.cancel(); setIsReading(false); }, [tangId]);
  useEffect(() => { localStorage.setItem("guowen-quiz-history", JSON.stringify(quizHistory)); }, [quizHistory]);
  useEffect(() => { localStorage.setItem("guowen-wrong-book", JSON.stringify(wrongBook)); }, [wrongBook]);
  ...
  const readTangContinuously = () => { if (!("speechSynthesis" in window)) return; window.speechSynthesis.cancel(); setIsReading(true); const queue = activeTang.sections.map((item) => item.original); let index = 0; const next = () => { if (index >= queue.length) { setIsReading(false); return; } const utterance = new SpeechSynthesisUtterance(queue[index++]); utterance.lang = "zh-TW"; utterance.rate = speechRate; utterance.onend = next; utterance.onerror = () => setIsReading(false); window.speechSynthesis.speak(utterance); }; next(); };
  const saveQuizResult = () => { const now = new Date().toISOString(); const wrong = activeTangLearning.quiz.filter((question, index) => quizAnswers[index] !== question.answer); const attempt: QuizAttempt = { id: `${activeTang.id}-${Date.now()}`, tangId: activeTang.id, title: activeTang.title, score: quizScore, total: activeTangLearning.quiz.length, wrong: wrong.map((question) => question.prompt), createdAt: now }; setQuizHistory((items) => [attempt, ...items].slice(0, 20)); setWrongBook((items) => { const next = items.filter((item) => !wrong.some((question) => item.key === `${activeTang.id}:${question.prompt}`)); const added = wrong.map((question) => ({ key: `${activeTang.id}:${question.prompt}`, tangId: activeTang.id, title: activeTang.title, type: question.type, prompt: question.prompt, answer: question.answer, explanation: question.explanation, lastWrongAt: now })); return [...added, ...next].slice(0, 40); }); setQuizSubmitted(true); };
