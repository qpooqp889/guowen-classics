    ...
    <main className="min-h-screen overflow-hidden bg-paper text-ink">
      <header className="relative z-20 border-b border-ink/10 bg-paper/95 backdrop-blur-sm">
        <div className="mx-auto flex max-w-[1440px] items-center justify-between px-5 py-4 lg:px-10">
          ...
          <button className="rounded-full p-2 text-moss hover:bg-moss/10 md:hidden" onClick={() => setShowMenu((value) => !value)} aria-label="開啟行動版選單">{showMenu ? <X size={22} /> : <Menu size={22} />}</button>
        </div>
        {showMenu && <div className="border-t border-ink/10 bg-paper px-5 py-4 font-sans text-sm md:hidden"><a className="mr-5" href="#classics" onClick={() => setShowMenu(false)}>篇章閱讀</a><a className="mr-5" href="#how-to-read" onClick={() => setShowMenu(false)}>讀法提示</a><a href="#sources" onClick={() => setShowMenu(false)}>資料來源</a></div>}
      </header>
      ...
      <section id="classics" className="mx-auto grid max-w-[1440px] gap-8 px-5 py-16 lg:grid-cols-[220px_1fr] lg:px-16 lg:py-24">
        ...
        <div className="paper-page relative min-w-0 border-y border-ink/10 p-6 sm:p-10 lg:border-y-2 lg:p-14">
          ...
          <div className="max-w-4xl"><div className="flex flex-wrap items-end justify-between gap-5"><div><p className="section-kicker">{active.theme}</p><h2 className="mt-3 font-serif text-4xl font-bold tracking-tight sm:text-5xl">{active.title}</h2><p className="mt-3 font-sans text-sm text-ink/55">{active.author}・{active.dynasty}</p></div><button className="listen-button" onClick={() => speak(active.sections.map((item) => item.original).join(" "))}><Headphones size={16} />朗讀全文</button></div>
